    ...
    function init() {
        $('#comingSoon_div').datagrid({              //数据表格
            ...
            toolbar: [{      //工具栏
                iconCls: 'icon-add',
                text: "增加电影",
                handler: comingSoon_add
            }, '-', {
                iconCls: 'icon-remove',
                text: '删除电影',
                handler: comingSoon_remove
            }, '-', {
                text: `
                    <input id="comingSoon_one"></input>
                    <div id="comingSoon_two" style="width:200px">
                ...
                text: '刷新',
                handler: comingSoon_reload
            }]
        });
        comingSoon_search();
    }
     
    function comingSoon_text(themes) {
        $.messager.show({
            title: '消息提示',
            msg: '将在3秒后自动关闭。',
            timeout: 3000,
    ...
    function comingSoon_add() {
        $('#comingSoon_window').dialog({
            width: 800,
            height: 400,
            title: "增加电影",
            buttons: [{
                ...
                iconCls: 'icon-save',
                handler: () => {
                    let art = $("#comingSoon_table").datagrid("getSelected");
                    $.post('/comingSoon/removeRepeat', { _id: art._id, movie_isfalse: 'true' }, function (data) {
                        $("#comingSoon_table").datagrid("reload")
                    })
                    $.post('/comingSoon/addcoming', $("#comingSoon_table").datagrid("getSelected"), function (data) {
                        $("#comingSoon_div").datagrid("reload");
                    })
                }
            }],
        })
        $('#comingSoon_table').datagrid({
            method: "post",
            url: "/comingSoon/coming_add",
            checkbox: "true",
            rownumbers: "true",
            fitColumns: "true",
            striped: "true",
    ...
    function comingSoon_remove() {
        if (!$("#comingSoon_div").datagrid("getSelected")) {
            $.messager.alert('消息提示', '请选择一条数据')
        } else {
            $.messager.confirm('确认', '您确认想要删除记录吗？', function (r) {
                if (r) {
                    let arr = $("#comingSoon_div").datagrid("getSelections");
                    for (let item of arr) {
                        $.post("/comingSoon/coming_remove", {
                            _id: item._id
                        }, (data) => {
                            comingSoon_text("删除成功")
                            $("#comingSoon_div").datagrid("reload")
                        })
                    }
                } else {
                    comingSoon_text('删除失败')
                }
            });
        }
    }
    ...
    function comingSoon_search() {
        $("#comingSoon_one").searchbox({
